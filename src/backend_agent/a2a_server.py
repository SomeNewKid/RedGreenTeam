"""Small A2A-compatible HTTP task server for the Backend Agent."""
...
_AGENT_NAME = "Backend Agent"
_DEFAULT_BUG_REPORT = (
    "The checkout API returns HTTP 500 after payment authorization, but the "
...
def serve_backend_agent(host: str, port: int, public_base_url: str) -> None:
    """Serve the Backend Agent over a small JSON-RPC HTTP surface."""
...
def _build_handler(public_base_url: str) -> type[BaseHTTPRequestHandler]:
    class _BackendAgentHandler(BaseHTTPRequestHandler):
        server_version = "BackendAgentA2A/1.0"
    ...
    return _BackendAgentHandler


def _build_agent_card(public_base_url: str) -> dict[str, object]:
    return build_agent_card(
        name=_AGENT_NAME,
        description="Assesses whether a bug report indicates a backend issue.",
        public_base_url=public_base_url,
        skills=[
            {
                "id": "assess_backend_bug",
                "name": "Assess Backend Bug",
                "description": (
                    "Return a backend likelihood percentage and concise reasons."
                ),
            }
        ],
    )
...
def _handle_message_send(request: dict[str, object]) -> dict[str, object]:
    ...
    task_id = f"backend-task-{uuid.uuid4()}"
    context_id = f"backend-context-{uuid.uuid4()}"
    task = build_task(
        task_id,
        context_id,
        "TASK_STATE_SUBMITTED",
        message_text="Backend assessment task submitted.",
    )
    _set_task(task_id, task)

    thread = threading.Thread(
        target=_run_backend_task,
...
def _run_backend_task(task_id: str, context_id: str, bug_report: str) -> None:
    _set_task(
        task_id,
        build_task(
            task_id,
            context_id,
            "TASK_STATE_WORKING",
            message_text="Backend assessment task is running.",
        ),
    )
    try:
        assessment = run_backend_agent(bug_report)
    ...
    _set_task(
        ...
        build_task(
            ...
            message_text="Backend assessment task completed.",
            artifacts=(artifact,),
        ),
    )
