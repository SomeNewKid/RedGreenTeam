...
if TYPE_CHECKING:
    from .sandbox_plan import AgentSpec, ResolvedSandboxPlan, SandboxRunSpec
...
class DockerProfile:
    ...
    denied_executable_paths: tuple[str, ...] = (
        "/opt/sandbox-agent/.venv/bin/pip",
        "/opt/sandbox-agent/.venv/bin/pip3",
        "/usr/local/bin/pip",
        "/usr/local/bin/pip3",
        "/usr/bin/apt",
        "/usr/bin/apt-get",
...
@dataclass(frozen=True)
class AgentImageConfiguration:
    """Resolved image and profile configuration for one agent."""
...
class DockerConfiguration:
    ...
    sandbox_run_spec: "SandboxRunSpec | None" = None
    agent_specs: tuple["AgentSpec", ...] = ()
    resolved_sandbox_plan: "ResolvedSandboxPlan | None" = None
    agent_image_configurations: tuple[AgentImageConfiguration, ...] = ()
...
@dataclass(frozen=True)
class AgentContainerRunResult:
    """Result of running one planned agent container."""
...
class DockerRunResult:
    ...
    stdout: str
    stderr: str
    agent_results: tuple[AgentContainerRunResult, ...] = ()
    ...
    def remove_container(self) -> None:
        ...
        remove_commands = [self.remove_command]
        if self.agent_results:
            remove_commands = [result.remove_command for result in self.agent_results]
